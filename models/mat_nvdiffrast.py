'''
@File    :   mat_nvdiffrast.py
@Time    :   2023/02/27 12:09:51
@Author  :   Zhen Li 
...
from utils.sample_util import   TINY_NUMBER, generate_dir, generate_fixed_samples, TINY_TINY_NUMBER
...
class MaterialModel(nn.Module):
    def __init__(self, conf, ids, extrinsics, optim_cam=False, gt_irf=True, gt_irrt=True):
        ...
        self.max_mip_level = (get_mip_level(8192))
        ...
        self.materials_a = (nn.Parameter(torch.ones((2048, 2048, 3)) * 0.5, requires_grad=True))
        self.materials_r = (nn.Parameter(torch.ones((4096, 4096, 1)) * 0.1, requires_grad=True))
    ...
    def forward(self, mvp, id, cam_position, stage=1):
        ...
        roughness_womipmap = dr.texture(self.materials_r[None, ...], texc, texd, filter_mode='linear')
        ...
        if stage==-1:
            # a simple trick to locate the light source for virtual regions.
            source = self.texture
            intensity = rgb_to_intensity(self.texture * (2**-self.conf.get_float('train.hdr_exposure')), dim=1).permute(0,2,3,1)
            self.texture = torch.where(intensity[...,0:1]>=0.5, source.permute(0,2,3,1), torch.tensor([0.0, 0.0, 0.0])).permute(0,3,1,2)
            # test_hdr = self.texture[0].permute(1,2,0).numpy()
            # cv2.imwrite("test.hdr",cv2.flip(test_hdr, 0))
            # exit(0)
            res = self.render(g_buffers[:,:,:,3:6].detach(), torch.zeros_like(albedo).cuda(), torch.ones_like(roughness).cuda()*0.01, g_buffers[:,:,:,0:3].detach()+1e-2*g_buffers[:,:,:,3:6].detach(), cam_position, irr)
            self.texture = source
        # lambertian-based albedo initing
        elif stage == 0:
            # irrf
            # with torch.no_grad():
            #     face, h, w, c = g_buffers.shape
            #     points = (g_buffers[:,:,:,0:3].detach()+1e-2*g_buffers[:,:,:,3:6].detach()).reshape(-1, 3)
            #     irr = hdr_recover(self.ir_radiance_network(points)).reshape(face,h,w,3) # shape : [b, 3]
            ...
            # source
            # face, h, w, c = g_buffers.shape
            # normal = g_buffers[:,:,:,3:6].detach().reshape(-1,3)
            # points = g_buffers[:,:,:,0:3].detach().reshape(-1,3)
            ...
            # diffuse = self.diffuse_reflectance(diffuse_lighting, light_dir_diff, normal, albedo.reshape(-1, 3), self.sample_type[0]) / self.sample_l[0]
            # diffuse = diffuse.reshape(face, h, w, 3)

            res = {
                'rgb':  diffuse ,
                'albedo': albedo,
                'normal': g_buffers[:,:,:,3:6].detach(),
                'position': (g_buffers[:,:,:,0:3].detach()+1e-1*g_buffers[:,:,:,3:6].detach())
            }
        # highlight-seg-based propagation
        elif stage == 1:
            res = self.render(g_buffers[:,:,:,3:6].detach(), albedo.detach(), roughness_womipmap, g_buffers[:,:,:,0:3].detach()+1e-2*g_buffers[:,:,:,3:6].detach(), cam_position, irr)
        # optim all
        elif stage == 2:
        ...
        res.update(
            {
                'empty_mask': mask,
                'roughness_womipmap': roughness_womipmap,
                'roughness': roughness
            }
        )
        return res
    ...
    def render(self, normal: torch.Tensor, albedo: torch.Tensor, roughness: torch.Tensor, points: torch.Tensor, cam_position: torch.Tensor, irr: torch.Tensor):
    ...
    def specular_reflectance(self, lighting, h, n, v, l, roughness, albedo=None):
        ...
        g1_ndv = ndv / torch.clamp( ndv *(1-k) + k , min=TINY_TINY_NUMBER)
        g1_ndl = ndl / torch.clamp( ndl *(1-k) + k , min=TINY_TINY_NUMBER)
        g = g1_ndl * g1_ndv

        ##brdf: f * d * g / (4*ndl*ndv)
        brdf = f * g / torch.clamp(4 * ndl * ndv, min=TINY_TINY_NUMBER)
        ##pdf : D*ndh / (4*vdh)
        ##equation: L = lighing * brdf * ndl
        return torch.sum( lighting * brdf * ndl * 4 * vdh / torch.clamp(ndh, TINY_TINY_NUMBER), dim=1)

        # ensure consistent to zhengqin's optixrender
        # pdf: a2 = r4, a2*ndh / ( (4 * M_PIf * (1 + (a2-1) * NoH)*(1 + (a2-1) * NoH) * VoH ) )
        # a2 = roughness.unsqueeze(1) * roughness.unsqueeze(1) * roughness.unsqueeze(1) * roughness.unsqueeze(1)
        # nom0 = ndh*ndh*(a2-1)+1
        # nom1 = ndv *(1-k)+k
        # nom2 = ndl *(1-k)+k
        # nom = torch.clamp(4*np.pi*nom0*nom1*nom2,TINY_NUMBER)
        # brdf = f*a2/nom
        # pdf = a2 * ndh / torch.clamp(4*np.pi* (1+(a2-1)*ndh)*(1+(a2-1)*ndh)*vdh, min=TINY_NUMBER)
        # return torch.sum(lighting * brdf * ndl / pdf , dim=1)
