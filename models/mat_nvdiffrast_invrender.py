'''
@File    :   mat_nvdiffrast_invrender.py
@Time    :   2023/02/27 12:09:28
@Author  :   Zhen Li 
...
from models.incidentNet import IRNetwork, MatNetwork, EnvmapMaterialNetwork
...
class MaterialModel(nn.Module):
    def __init__(self, conf, ids, extrinsics, optim_cam=False, gt_irf=True, gt_irrt=True):
        ...
        self.material_network = EnvmapMaterialNetwork()
    ...
    def forward(self, mvp, id, cam_position, stage=1):
        ...
        materials_dict = self.material_network(g_buffers[:,:,:,0:3].detach())
        albedo = materials_dict['diffuse_albedo']
        roughness = materials_dict['roughness']

        res = self.render(g_buffers[:,:,:,3:6].detach(), albedo, roughness, g_buffers[:,:,:,0:3].detach()+1e-2*g_buffers[:,:,:,3:6].detach(), cam_position, irr)
        

        return dict(materials_dict, **res)
