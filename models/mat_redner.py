...
from models.incidentNet import IRNetwork
...
class MaterialModel(nn.Module):
    def __init__(self, conf, ids, extrinsics, optim_cam=False, gt_irf=True):
        ...
        # self.materials = nn.Parameter(torch.ones((3600, 3600, 3))*0.5, requires_grad=True)
        self.materials_albedo = []
        for object in self.object_list:
            # self.materials_albedo.append(nn.Parameter( torch.ones_like(object.material.diffuse_reflectance.texels)*0.5, requires_grad=True))
            self.materials_albedo.append(nn.Parameter( torch.ones((2048, 2048, 3))*0.5, requires_grad=True))
        self.materials_albedo = nn.ParameterList(self.materials_albedo)

        self.materials_roughness = []
        for object in self.object_list:
            # self.materials_roughness.append(nn.Parameter( torch.ones(list(object.material.diffuse_reflectance.texels.shape[:-1]) + [1])*0.5, requires_grad=True))
            self.materials_roughness.append(nn.Parameter( torch.ones((2048, 2048, 1))*0.4, requires_grad=True))
        self.materials_roughness = nn.ParameterList(self.materials_roughness)
        ...
        if gt_irf:
            ...
            texture = cv2.imread(self.path_traced_mesh.replace("_smooth.obj","_hdr_ccm.hdr"), -1)[:,:,::-1]
    ...
    def forward(self, cam_to_world, id, cam_position):
        ...
        for index in range(len(self.object_list)):
            self.object_list[index].material.diffuse_reflectance = pyredner.Texture(self.materials_albedo[index])
            self.object_list[index].material.generic_texture = pyredner.Texture(self.materials_roughness[index])
        if self.optim_cam:
        ...
        g_buffers = pyredner.render_g_buffer(scene=scene, \
            channels=[
                ...
                pyredner.channels.geometry_normal], num_samples=[4,2])
        
        img = self.render(g_buffers[:,:,7:10].detach(), g_buffers[:,:,0:3], g_buffers[:,:,3:4], g_buffers[:,:,4:7].detach()+1e-2*g_buffers[:,:,7:10].detach(), cam_position)
        return img.unsqueeze(0)
        # return g_buffers[:,:,7:10].detach().unsqueeze(0)
    ...
    def render(self, normal: torch.Tensor, albedo: torch.Tensor, roughness: torch.Tensor, points: torch.Tensor, cam_position: torch.Tensor):
        ...
        light_dir_diff = generate_dir(normal, self.sample_l[0], mode=self.sample_type[0])     # shape: [env_h*env_w, n_sample, 3]
        with torch.no_grad():
        ...
        h_dir_specular = generate_dir(normal, self.sample_l[1], self.sample_type[1], roughness)
...
if __name__=="__main__":
    conf = ConfigFactory.parse_file('./configs/mat.conf')
    mm = MaterialModel(conf).cuda()
