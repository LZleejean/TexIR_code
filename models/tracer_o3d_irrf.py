'''
@File    :   tracer_o3d_irrf.py
@Time    :   2023/02/27 12:10:14
@Author  :   Zhen Li 
...
class TracerO3d(nn.Module):
    def __init__(self, conf, AABB=None, is_hdr_texture=False):
        super().__init__()
        self.ir_radiance_network = MatNetwork(**conf.get_config('models.irrf_network'), AABB=AABB)
        ...
        self.std_jit = conf.get_float('train.std_jit')
    ...
    def forward(self, points, normals, resolution, isnot_first_val=False):
        ...
        if not isnot_first_val:
            ...
            predicted_mask_ir = self.ir_radiance_network(points) # shape: (b, 3)
            # predicted_mask_ir = predicted_mask_ir.reshape(b, resolution[0],resolution[1], 3)
            # predicted_mask_ir[~mask,:] = 0
            predicted_mask_ir_jit = self.ir_radiance_network(points + torch.normal(mean=0, std=self.std_jit, size=points.shape, device="cuda")) # shape: (b, 3)

            res = {
                'gt':  gt_irr ,
                'pred': predicted_mask_ir,
                'pred_jit': predicted_mask_ir_jit
            }
