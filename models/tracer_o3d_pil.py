'''
@File    :   tracer_o3d_pil.py
@Time    :   2023/02/27 12:10:31
@Author  :   Zhen Li 
...
from models.incidentNet import IRNetwork, MatNetwork, PILNetwork
from utils.sample_util import *

class TracerO3d(nn.Module):
    def __init__(self, conf, AABB=None, is_hdr_texture=False):
        super().__init__()
        self.ir_radiance_network = PILNetwork(**conf.get_config('models.irrf_network'), AABB=AABB)
        ...
        self.path_lut = os.path.join(os.path.abspath(__file__), 'bsdf_256_256.bin')
    ...
    def forward(self, points, normals, resolution, isnot_first_val=False):
        ...
        n_r = 1
        roughness = torch.rand(b, n_r).reshape(-1, 1).cuda()   # shape: [b, 1]
        # points = points.repeat(n_r, 1)   # shape: [b*n_r, 3]
        # normals = normals.repeat(n_r, 1)   # shape: [b*n_r, 3]
        h_samples = generate_dir(normals, resolution[0]*resolution[1], None, 'importance', roughness).reshape(b, resolution[0], resolution[1], 3)

        n_wo = 1
        wo = generate_dir(normals, n_wo, samples=None, mode='uniform', roughness=None, pre_mode='independent')  # shape: [b, n_wo, 3]

        vdh = torch.clamp(torch.sum( wo.unsqueeze(1) * h_samples, dim=-1,keepdim=True), 0.0, 1.0)    # shape: [b, n_sample[0], n_sample[1], 1]
        directions = 2  * vdh * h_samples - wo.unsqueeze(1) # shape: (b, n_sample[0], n_sample[1], 3)
        ...
        if not isnot_first_val:
            ...
            gt_ir[ner_mask,:] = 0
            # # consider decay
            # distances = np.repeat(hit[..., np.newaxis], 3, -1)
            # attenuation = 1.0 / (1. + 0.14 * distances + 0.07*distances*distances)
            # gt_ir[mask,:] = gt_ir[mask,:] * attenuation[mask,:]
            # gt_irr = torch.mean(gt_ir.reshape(b, -1, 3), dim=-2)
            ndl = torch.clamp(torch.sum( normals.unsqueeze(1) * directions.reshape(b, -1, 3), dim=-1, keepdim=True), 0.0, 1.0)    # shape: [b, n_sample, 1]
            gt_irr = torch.sum(gt_ir.reshape(b, -1, 3).cuda() * ndl, dim=1) * 2 * np.pi / (resolution[0]*resolution[1])


            predicted_mask_ir = self.ir_radiance_network(points, wo.squeeze(1), roughness) # shape: (b, 3)
            # predicted_mask_ir = predicted_mask_ir.reshape(b, resolution[0],resolution[1], 3)
            # predicted_mask_ir[~mask,:] = 0

            return gt_irr, predicted_mask_ir
        else:
            predicted_mask_ir = self.ir_radiance_network(points, wo.squeeze(1), roughness) # shape: (b, 3)
            # predicted_mask_ir = predicted_mask_ir.reshape(b, resolution[0],resolution[1], 3)
            return predicted_mask_ir
    ...
    def generate_dir(self, normals, resolution):
        ...
        # ######### test sample directions 1, via z as up direction
        # theta = torch.linspace(-np.pi,np.pi,256).cuda()
        # phi = -torch.linspace(-np.pi/2.0,np.pi/2.0,128).cuda()
        # phi,theta = torch.meshgrid([phi,theta])

        # v_x = torch.cos(phi) * torch.sin(theta)
        # v_y = torch.sin(phi)
        # v_z = -torch.cos(phi) * torch.cos(theta)
        # samples = nn.Parameter(torch.stack([v_x,v_y,v_z],dim=-1).unsqueeze(0), requires_grad=False)  #size: (b, h, w ,3)

        # L = V * (samples[:,:,:,0:1]) \
        #                 + normals * (samples[:,:,:,1:2]) \
        #                 +  U* samples[:,:,:,2:3] # [batch, num_samples, 1, 3]

        # ######### test sample direction 2
        # theta = torch.linspace(-np.pi,np.pi,256).cuda()
        # phi = -torch.linspace(-np.pi/2.0,np.pi/2.0,128).cuda()
        # phi,theta = torch.meshgrid([phi,theta])
        ...
        # L = nn.Parameter(torch.stack([v_x,v_y,v_z],dim=-1).unsqueeze(0), requires_grad=False)  #size: (b, h, w ,3)
