...
class NovelViewRunner():
    def __init__(self,**kwargs):
        ...
        self.editing_dir = os.path.join(self.plots_dir,'novel_view')
    ...
    def plot_to_disk_cube(self):
        ...
        index = -1
        for i in range(len(self.train_dataset.extrinsics_list)):
            cam_to_world = self.train_dataset.extrinsics_list[i].cuda()
            cam_position = self.train_dataset.cam_position_list[i].cuda()

            res = self.model(cam_to_world, 0, cam_position, True)
            ...
            plt.plot_mat(self.editing_dir, 0, pred_img, "novel_view_{}".format(i), False)
