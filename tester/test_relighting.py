'''
@File    :   test_relighting.py
@Time    :   2023/02/27 12:11:02
@Author  :   Zhen Li 
...
class RelightingRunner():
    def __init__(self,**kwargs):
        ...
        self.editing_dir = os.path.join(self.plots_dir,'relighting')
        ...
        self.model = utils.get_class(self.conf.get_string('test.model_class'))(conf=self.conf, \
            cam_position_list = self.train_dataset.cam_position_list, checkpoint_material=self.plots_dir, relighting=True)
    ...
    def plot_to_disk_cube(self):
        ...
        for i in range(len(self.train_dataset.ids)):
            ...
            plt.plot_mat(self.editing_dir, 0, pred_img, "relighting_{}".format(i), False)
    ...
    def plot_to_disk(self):
        ...
        for i in range(len(self.train_dataset.ids)):
            ...
            res = self.model(cam_to_world, derived_id, cam_position)
            pred_img = res['rgb'].cpu().detach()  # shape: (h, w, 3)
