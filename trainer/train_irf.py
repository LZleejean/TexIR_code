...
class IRFTrainRunner():
    def __init__(self,**kwargs):
        ...
        self.expname = 'IRF-' + kwargs['expname']
        ...
        self.irf_optimizer = torch.optim.Adam(self.model.incident_radiance_network.parameters(),
    ...
    def plot_to_disk(self):
        ...
        for data_index, (one_sample) in enumerate(self.plot_dataloader):
            ...
            if self.first_val:
                res = self.model(points, normals, self.val_resolution)
                batch_gt_ir, batch_predicted_ir = res['gt'], res['pred']
                b, num, c = batch_gt_ir.shape
                batch_gt_ir = torch.sum(batch_gt_ir, dim=1)/num   # multiply 2 if use render pano because only half directions are used.
                batch_predicted_ir = torch.sum(batch_predicted_ir, dim=1)/num
                ...
                res = self.model(points, normals, self.val_resolution, True)
                batch_predicted_ir = res['pred']
                b, num, c = batch_predicted_ir.shape
                batch_predicted_ir = torch.sum(batch_predicted_ir, dim=1)/num
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.nepochs + 1):
            ...
            for data_index, (one_sample) in enumerate(self.train_dataloader):
                ...
                points = one_sample['point'].float().cuda()
                normals = one_sample['normal'].float().cuda()
                res= self.model(points, normals, self.train_resolution)
                radiance_loss = self.irf_loss(res)
