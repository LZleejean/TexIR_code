'''
@File    :   train_material_neilf_syn.py
@Time    :   2023/02/27 12:11:48
@Author  :   Zhen Li 
...
class MatNeilfTrainSynRunner():
    def __init__(self,**kwargs):
        ...
        self.mat_loss = utils.get_class(self.conf.get_string('train.irf_loss_class'))()
        if self.conf.get_bool('train.optim_cam'):
            ...
            #                                         lr=self.conf.get_float('train.mat_learning_rate'))
        self.mat_scheduler = torch.optim.lr_scheduler.MultiStepLR(self.mat_optimizer,
                                                self.conf.get_list('train.mat_sched_step', default=[30, 30]),
                                                gamma=self.conf.get_float('train.mat_sched_factor', default=0.2))
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.nepochs + 1):
            ...
            for data_index, (gt_item) in enumerate(self.train_dataloader):
                ...
                gt_color_grad = gt_item['rgb_grad'].float().cuda()
                gt_color_grad = gt_color_grad.reshape(-1, h, w, 1)
                
                preds = self.model(gt_extrinsic[0], id[0], cam_position[0], 0)

                loss = self.mat_loss(preds, gt_color, gt_color_grad)
